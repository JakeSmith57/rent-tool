...
BOOLEAN COERCION (2026-09-20 fix): flags that arrive from sqlite/CSV as
STRINGS are now parsed with _truthy_series() instead of pandas'
.astype(bool). `"0"` is a non-empty Python string, so `.astype(bool)`
returns True for it -- which made is_condo_coop True for every PLUTO row
whose flag came back as a string, and is_condo_coop=True disqualifies the
pre-1974/6+-unit fallback in compute_stabilization_confidence(), silently
suppressing every "low"-tier result. The DHCR half of that same expression
was already doing this correctly with .isin([...]); both halves now share
one helper.
"""
...
# Values that mean "true" when a 0/1 flag has been round-tripped through
# sqlite, a CSV, or pandas' object dtype. Anything else (including "0",
# "", "nan", "None", "false") is false.
_TRUTHY_STRINGS = {"1", "1.0", "true", "t", "yes", "y"}


def _truthy_series(s: pd.Series) -> pd.Series:
    """Coerce a possibly-string, possibly-null 0/1 flag column to real bools.

    NEVER use .astype(bool) for this: the string "0" is truthy in Python,
    so .astype(bool) turns a whole column of "0"s into True. See the module
    docstring's "BOOLEAN COERCION" note.
    """
    if s is None:
        return pd.Series(dtype=bool)
    # Genuine numeric/bool dtypes can be compared directly; only object/
    # string columns need the string-set test.
    if pd.api.types.is_bool_dtype(s) or pd.api.types.is_numeric_dtype(s):
        return s.fillna(0).astype(float) != 0
    return s.astype(str).str.strip().str.lower().isin(_TRUTHY_STRINGS)
        ...
        .agg(
            {
                ...
                "is_condo_coop": lambda s: str(int(any(str(v).strip().lower() in _TRUTHY_STRINGS for v in s))),
...
def main() -> None:
    ...
    # Both halves go through _truthy_series() -- see the module docstring's
    # "BOOLEAN COERCION" note for why .astype(bool) was wrong here.
    is_condo_pluto = (
        _truthy_series(reg["is_condo_coop_pluto"])
        if "is_condo_coop_pluto" in reg.columns
        else pd.Series(False, index=reg.index)
    )
    is_condo_dhcr = _truthy_series(reg["is_condo_coop"])
    reg["is_condo_coop"] = (
        is_condo_pluto.reindex(reg.index).fillna(False)
        | is_condo_dhcr.reindex(reg.index).fillna(False)
    ).astype(int)
    ...
    print(f"is_condo_coop=1 rows: {int(final['is_condo_coop'].sum())} of {len(final)} "
          "(a value equal to the row count almost certainly means a boolean-coercion "
          "regression -- see the module docstring's BOOLEAN COERCION note)")
